...
def main():
    ...
    while True:
        ...
        try:
            ...
            tempstr = str("{0}{1}F".format(int(round(temperature['temp'])), char(176)))
            print( tempstr)
            w4, h4 = font24.getsize(tempstr)
            drawblack.text((10, 70), tempstr, font = font24, fill = 0)
            drawred.text((10+w4, 70), "'", font = fontweather, fill = 0) 
            drawblack.text((150, 70), str("{0}{1} | {2}{3}".format(int(round(temperature['temp_min'])), char(176), int(round(temperature['temp_max'])), char(176))), font = font24, fill = 0)
